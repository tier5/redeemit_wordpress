...
$check_expiry_query = $wpdb->get_results("select * from reedemer_offer where id=".$offer_id." && end_date >= CURDATE()");
if(is_array($check_expiry_query) && count($check_expiry_query) == 0){
...
$right_sidebar = "SELECT ro.*,usr.location, SQRT(POW(69.1 * (ro.latitude - $lat_dis), 2) + POW(69.1 * ($lon_dis - ro.longitude) * COS(latitude / 57.3), 2)) AS distance FROM `reedemer_offer` as ro INNER JOIN `users` as usr ON ro.created_by = usr.id  WHERE ro.status = 1 && ro.published = 'true' && ro.id != $offer_id && ro.created_by != $user_id && ro.cat_id = $cat_id && ro.end_date >= CURDATE() group by ro.created_by HAVING distance < $min_distance  ORDER BY RAND() limit 4";	
	
$right_sidebar_rand= "SELECT ro.*,usr.location, SQRT(POW(69.1 * (ro.latitude - $lat_dis), 2) + POW(69.1 * ($lon_dis - ro.longitude) * COS(latitude / 57.3), 2)) AS distance FROM `reedemer_offer` as ro INNER JOIN `users` as usr ON ro.created_by = usr.id  WHERE ro.status = 1 && ro.published = 'true' && ro.id != $offer_id && ro.created_by != $user_id && ro.end_date >= CURDATE() group by ro.created_by HAVING distance < $min_distance  ORDER BY RAND() limit 4";	
...
$bottom_grid = "SELECT ro.*,usr.location, SQRT(POW(69.1 * (ro.latitude - $lat_dis), 2) + POW(69.1 * ($lon_dis - ro.longitude) * COS(latitude / 57.3), 2)) AS distance FROM `reedemer_offer` as ro INNER JOIN `users` as usr ON ro.created_by = usr.id  WHERE ro.status = 1 && ro.published = 'true' && ro.id != $offer_id && ro.end_date >= CURDATE() HAVING distance < $min_distance ORDER BY RAND() limit 100";	
...
$bottom_grid = "SELECT ro.*,usr.location, SQRT(POW(69.1 * (ro.latitude - $lat_dis), 2) + POW(69.1 * ($lon_dis - ro.longitude) * COS(latitude / 57.3), 2)) AS distance FROM `reedemer_offer` as ro INNER JOIN `users` as usr ON ro.created_by = usr.id  WHERE ro.status = 1 && ro.published = 'true' && cat_id = $cat_id && id != $offer_id && ro.end_date >= CURDATE() HAVING distance < $min_distance ORDER BY RAND() limit 100";	
$bottom_grid1=$wpdb->get_results($bottom_grid,ARRAY_A);
    ...
    <div class="row">
         <div class="col-md-9 col-sm-9 product-details">
			...
			?>
				...
				<a href="#" id="pay_it" user = "<?php echo $get_user1[0]['id'] ; ?>" offer_id = "<?php echo $offer_details_res[0]['id'] ; ?>">Buy Offer</a>
            ...
            <div class="pro-content">
                ...
                <p><?php if(empty($offer_details_res[0]['brand_details'])){echo 'No information 							There';}else{echo $offer_details_res[0]['brand_details'];} ?>
				</p>
                <h3>What You Get</h3>
					...
					<div class="home-page-deal-pic">
                        ...
                        <div class="tab-product">
                            <div class="product-title">
								<?php if(strlen($right_sidebar['offer_description']) > 50) { 
									echo substr($right_sidebar['offer_description'],0,49)."...";
								} else { 
        ...
        <div class="row">
            ...
            foreach ($bottom_grid1 as $grid_value) {
                ...
                <?php 
					if($k%3 == 0)
							{
								echo   '</div><div class="row more-deal-row">' ;
							}
						$k++;
               	}
